...
// ===== Stick Control =====
const STICK_RADIUS = 60;        // px (half of pad)
const KNOB_RADIUS = 14;
const EE_STEP_CM = 0.15;        // motion per frame (tune this)

const stick = document.getElementById("eeStick");
const knob = document.getElementById("eeStickKnob");

function setKnob(dx, dy) {
  knob.style.left = `${STICK_RADIUS + dx - KNOB_RADIUS}px`;
  knob.style.top  = `${STICK_RADIUS - dy - KNOB_RADIUS}px`;
}
...
stick.addEventListener("pointermove", e => {
  ...
  const cx = rect.left + STICK_RADIUS;
  const cy = rect.top + STICK_RADIUS;

  let dx = e.clientX - cx;
  let dy = cy - e.clientY; // invert so up = +
  ...
  state.stick.dx = dx / STICK_RADIUS; // normalize to [-1,1]
  state.stick.dy = dy / STICK_RADIUS;
...
stick.addEventListener("pointerleave", releaseStick);


function updateEndEffectorFromStick() {
	if (state.stick.dx === 0 && state.stick.dy === 0) return;

	// Save current target
	const prevX = state.endEffector.x;
	const prevY = state.endEffector.y;

	// Propose a new target (do NOT commit yet)
	const nextX = prevX + state.stick.dx * EE_STEP_CM;
	const nextY = prevY + state.stick.dy * EE_STEP_CM;

	const ok = applyIK_EndEffector(
		nextX,
		nextY,
		null,      // keep current orientation
		"auto",
		null
	);

	// Only commit the target if IK succeeded
	if (ok) {
		state.endEffector.x = nextX;
		state.endEffector.y = nextY;
	}
}

function stickLoop() {
  updateEndEffectorFromStick();
  requestAnimationFrame(stickLoop);
}
...
cv.addEventListener("pointermove", (e) => {
	...
	if (dragKind === "J0") {
		...
		const current0 = (state.angles.j0 * 180) / Math.PI;
		...
		const currentPhi1 = ((state.angles.j0 + state.angles.j1) * 180) / Math.PI;
		phi1Deg = normalizeToNearest(phi1Deg, currentPhi1);

		// Convert to LOCAL J1 angle: angle1 = phi1 - angle0
		const angle0Deg = (state.angles.j0 * 180) / Math.PI;
		...
		const currentPhi2 = ((state.angles.j0 + state.angles.j1 + state.angles.j2) * 180) / Math.PI;
		phi2Deg = normalizeToNearest(phi2Deg, currentPhi2);

		// LOCAL J2: angle2 = phi2 - (angle0 + angle1)
		const base12Deg = ((state.angles.j0 + state.angles.j1) * 180) / Math.PI;
